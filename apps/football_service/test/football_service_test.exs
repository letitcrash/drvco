defmodule FootballServiceTest do
  ...
  @leagues_list [%{league: "D1", seasons: [%{season: "201617"}]}, %{league: "E0", seasons: [%{season: "201617"}]}, %{league: "SP1", seasons: [%{season: "201516"}, %{season: "201617"}]}, %{league: "SP2", seasons: [%{season: "201516"}, %{season: "201617"}]}]
  ...
  test "fetch the result for specific league and season from store" do
    {:ok, [ first | tail ]} = Store.get_scores(@league, @season, format: :json)
  ...
  test "invalid data from storage tests" do
    assert {:error, "League or Season is not available"} == Store.get_scores(@league, @unavailable_season)
    assert {:error, "League or Season is not available"} == Store.get_scores(@unavailable_league, @season)
    assert {:error, "League or Season is not available"} == Store.get_scores(@unavailable_league, @unavailable_season)
